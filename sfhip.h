...
YOU MUST CALL:
  ...
  When your PHY Receives Packet, call:
    ...
    int sfhip_accept_packet( sfhip * hip, sfhip_phy_packet_mtu * data,
      int length );
...
typedef struct
{
	...
	sfhip_address remote_address; // this being nonzero is what determines if a socket is active or not.
	hipbe16       local_port;
	hipbe16       remote_port;
	uint32_t      seq_num;
	uint32_t      ack_num;
...
sfhip_length_or_tcp_code sfhip_tcp_event( sfhip * hip, int sockno, uint8_t * ip_payload, int ip_payload_length, int max_out_payload, int acked );
void sfhip_tcp_socket_closed( sfhip * hip, int sockno );
#endif
...
int sfhip_send_tcp_packet( sfhip * hip, sfhip_phy_packet_mtu * pkt, sfhip_length_or_tcp_code payload_length, tcp_socket * sock )
{
	...
	if( flags & SFHIP_TCP_SOCKETS_FLAG_SYN )
	{
		payload_length = 4;
		optionadd = 4;
		((hipbe32 *)(tcp+1))[0] = HIPHTONL( 0x02040000 |
	...
	void * ip_payload, int ip_payload_length )
{
	...
	if( ts == tsend || (flags & SFHIP_TCP_SOCKETS_FLAG_SYN) )
	{
		...
		if( ts == tsend )
		{
			...
			if( flags & SFHIP_TCP_SOCKETS_FLAG_SYN )
			{
				...
				o = sfhip_tcp_accept_connection( hip, sockno, HIPNTOHS( tcp->destination_port ), sender );
			}
...

	{
		...
		if( ts->pending_send_size == 0 )
			cansend = SFHIP_MTU - sizeof( sfhip_mac_header ) -
